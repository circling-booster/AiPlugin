...
(function (root) {
    class AudioPlayer {
        ...
        async initPitchShifter() {
            try {
                // [수정됨] AiPlugin 환경 호환성 확보: chrome.runtime.getURL 제거
                // 로컬 서버(localhost:5010) 또는 주입된 스크립트 경로에서 rubberband-processor.js 찾기
                let workletUrl = 'http://localhost:5010/rubberband-processor.js';
                
                // 만약 페이지에 이미 주입된 스크립트 태그가 있다면 그 경로를 우선 사용
                const scriptEl = document.querySelector('script[src*="rubberband-processor.js"]');
                if (scriptEl) {
                    workletUrl = scriptEl.src;
                }

                console.log("[Player] Loading AudioWorklet from:", workletUrl);
